    ...
    <section class="sidebar scroll" :class="{ 'icons-only': collapse }">
        ...
        <ul class="nav nav-pills flex-column mb-auto" id="modules">
            <li v-for="button in buttons">
                <a :href="button.href" class="nav-link" :class="{ active: active == button.href.substring(1) }"> <i
                        :class="button.icon" :title="button.name" data-bs-toggle="tooltip"></i> {{ button.name }} </a>
            </li>
        </ul>
        <hr />
        <div class="dropdown">
            <a href="#" id="config" class="d-flex align-items-center text-white text-decoration-none dropdown-toggle"
                data-bs-toggle="dropdown" aria-expanded="false"> <i class="bi bi-gear" style="margin-right: 0.5rem"></i>
                <strong>Configuración</strong> </a>
